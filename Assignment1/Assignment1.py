...
def task_5(pos_prior , neg_prior , reviews , pos_probs , neg_probs):
    positive_likelihood = 0
    negative_likelihood = 0
   #here we instatiate two values for the positive and negative likelihood
    
    #for loop used to first check that the value of the words of the review are not none in the wordlists
...
def task_6(target , data , pos_prior , neg_prior , pos_probs , neg_probs):
    print("Task 6")
    ...
    for k in range(0,10):
        ...
        for i  in range(len(data)):
          #print(i,data[i])      
          prediction = task_5(pos_prior, neg_prior, data[i], pos_probs, neg_probs)
          #print(target[i])
...
training_data, training_labels, testing_data, testing_labels ,positive_train_num , negative_train_num= task_1(df)

#using task 2 to generate the target words from the training review set and returns a list of words
training_words = task_2(training_data)

#this line does the same as above for the testing review data
test_words = task_2(testing_data)
      
training_set_pos_review_words, training_set_neg_review_words = task_3(training_data , training_labels , test_words)
...
classifier = task_5(pos_prior, neg_prior, training_data, pos_probabilities, neg_probabilities)

task_6(testing_labels , testing_data , pos_prior , neg_prior, pos_probabilities , neg_probabilities)